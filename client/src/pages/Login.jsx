...
const Login = () => {
    ...
    const handleGoogleSignIn = () => {
            ...
            try {
                window.google.accounts.id.prompt((notification) => {
                    if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
                        // Fallback: render temporary hidden Google button and trigger click if prompt was blocked
        ...
        // 2. Fallback: prompt Google account picker / email prompt cleanly
        Swal.fire({
            title: 'Choose Google Account',
            text: 'Click your Google account to log in to PPG HUB:',
            input: 'email',
            inputPlaceholder: 'user@ppg.edu.in',
            showCancelButton: true,
            confirmButtonText: 'Select & Sign In',
            confirmButtonColor: '#2563eb',
            ...
            inputValidator: (value) => {
                if (!value || !value.includes('@')) {
                    return 'Please enter a valid Google account email!';
                }
            }
        }).then(({ value: selectedEmail }) => {
            if (selectedEmail) {
                processGoogleAuthPayload({ email: selectedEmail.trim() });
            }
        });
    };

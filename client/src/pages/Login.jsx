...
const Login = () => {
    ...
    const handleGoogleSignIn = async () => {
        const { value: email } = await Swal.fire({
            title: `
                <div class="flex flex-col items-center gap-2">
                    ...
                    <span class="text-xl font-bold text-gray-800">Choose a Google Account</span>
                </div>
            `,
            html: '<p class="text-xs text-gray-500 mb-2">Select or enter your official PPG employee email address to proceed to <b>PPG iTech - HUB</b>:</p>',
            input: 'email',
        ...
        setLoading(true);
        try {
            const data = await googleLogin(email.trim());
            ...
            setLoading(false);
            const errorMessage = error.response?.data?.message || 'Access Denied: The provided email is not registered as an official employee email.';
            Swal.fire({
                icon: 'error',
                title: 'Access Denied',
                text: errorMessage,
            ...
            >
                ...
                <div className="space-y-6">
                    <motion.button
                        ...
                        className="w-full py-4 px-6 rounded-2xl bg-white text-gray-800 font-bold text-sm shadow-xl flex items-center justify-center gap-3 hover:bg-gray-50 transition-all duration-200 group cursor-pointer border border-white/50"
                    >
                        {loading ? (
                            <span className="h-5 w-5 border-2 border-blue-600/30 border-t-blue-600 rounded-full animate-spin" />
                        ) : (
                            <>
                                ...
                                <span className="tracking-wide group-hover:text-blue-600 transition-colors">
                    ...
                    <p className="text-[10px] text-sky-200/80 text-center font-medium leading-relaxed px-4">
                        🔒 Note: Only authorized employees with registered email addresses are allowed to log in.
                    </p>
                </div>
